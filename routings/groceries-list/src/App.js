...
function App() {
  ...
  npx json-server -p 3500 -w data/db.json */
	...
	useEffect(() => {
		...
		// setTimeout - to simulate real API
		setTimeout(() => {
	...
	const addItem = async (item) => {
		...
		// API for POST request (to add an item):
		const postOptions = {
			method: 'POST',
			headers: {
	...
	const handleCheck = async (id) => {
		...
		const myItem = listItems.filter((item) => item.id === id); // get the item that being checked

		// API for PUTCH request (to update an item):
		const updateOptions = {
			method: 'PATCH',
			headers: {
				'Content-Type': 'application/json',
	...
	const handleDelete = async (id) => {
		...
		// API for DELETE request (to delete an item):
